...
type Props = { logs: ProcessingLog[]; onRefresh: () => void; busy?: boolean; compact?: boolean }
...
export function HistoryTable({ logs, onRefresh, busy, compact }: Props) {
  ...
  return (
    <section className="surface history-surface">
      <div className="section-header"><div><p className="overline">Activité</p><h2>Traitements récents</h2><p>Décisions enregistrées par le workflow n8n.</p></div><button type="button" className="button ghost" disabled={busy} onClick={onRefresh}>Actualiser</button></div>
      {visibleLogs.length === 0 ? <div className="empty-state compact"><span className="empty-icon">↗</span><h3>Aucun traitement</h3><p>Testez un email via n8n pour alimenter l’historique.</p></div> : (
